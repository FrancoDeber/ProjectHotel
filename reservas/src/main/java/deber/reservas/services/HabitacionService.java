...
import deber.reservas.repositories.HabitacionRepository;
...
public class HabitacionService {
    ...
    private final Class<Habitacion> entityClass = Habitacion.class;
    ...
    public String createEntity(Map<String,String> request) {
        try {
            Habitacion entity = entityClass.getDeclaredConstructor().newInstance();
            mapToEntity(request, entity);
            entity.setHotel(hotelService.getEntityById(Integer.parseInt(request.get("idHotel"))));
    ...
    public String updateEntity(Habitacion entity, Map<String,String> request){
        try{
            mapToEntity(request, entity);
            entity.setHotel(hotelService.getEntityById(Integer.parseInt(request.get("idHotel"))));
            repository.save(entity);
        ...
        return "Actualizada la habitacion con exito";
    }
    

    //ELIMINAR Habitacion
    public String deleteEntity(Habitacion entity){
        repository.delete(entity);
        return entityClass.getSimpleName() + " eliminado con exito";
    }

    //Sobreescribre el valor de Habitacion a partir de los valores del Request
    //Si un campo no esta en el Request y no es NonNull entonces le pone Null al valor de la Entity
    protected void mapToEntity(Map<String, String> request, Habitacion entity) {
        try {
    ...
    // Retorna el objeto de la Habitacion a partir del ID del objeto, comprueba que existe y sino tira CustomException
    public Habitacion getEntityById(int id) {
        Optional<Habitacion> entity = repository.findById(id);
        if (entity.isEmpty()) {
